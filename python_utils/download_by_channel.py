...
def fetch_channels(channel_data):
    """
    Fetches channel information given an array of (channel_id, read_key) pairs.
    
    Args:
        channel_data (list of tuples): Each tuple contains (channel_id, read_key).
    
    Returns:
        list: A list of channel information, including fields.
    """
    channel_list = []
    channel_number = 1

    for channel_id, read_key in channel_data:
        # Construct the URL
        url2 = f"https://api.thingspeak.com/channels/{channel_id}/feeds.json?api_key={read_key}&results=0"
        fields = []
        ...
        try:
            ...
            name = feed_data['channel'].get('name', 'Unknown')
            id_ = feed_data['channel'].get('id', 'Unknown')
            ...
            print(f"Error fetching fields from channel ID {channel_id}: {e}")
            continue

        # Save the channel information with the fields
        channel_list.append([id_, name, read_key, channel_number, fields])
        channel_number += 1
...
if __name__ == "__main__":
    ...
    channels = fetch_channels(read_keys)
